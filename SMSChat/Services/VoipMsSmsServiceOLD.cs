...
{
    public class VoipMsSmsServiceOLD
    {
        ...
        public VoipMsSmsServiceOLD(HttpClient httpClient, string bearerToken)
        {
        ...
        public async Task<IEnumerable<SmsMessage>> GetSmsMessagesAsync(string did)
        {
           // var url = $"{_apiBaseUrl}?method=getSMS&did={did}&api_username=paul.chavaux@gmail.com&api_password=uykMdVwGUT5L8n!&from=2024-11-06";
            ...
            if (response.IsSuccessStatusCode)
            {
                var result = await response.Content.ReadFromJsonAsync<VoipMsSmsResponseOLD>();
                return result?.Sms;
            }
            return null;
        }
